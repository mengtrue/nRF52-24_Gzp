...
uint32_t esb_init( void )
{
    ...
    VERIFY_SUCCESS(err_code);
		...
		{
		    err_code = nrf_esb_set_rf_channel(4);
		    VERIFY_SUCCESS(err_code);
		}
		
		hand = 2;
		test_spis_rx_data[0] = hand;
		for(int i = 1; i < TEST_SPIS_RX_DATA_LENGTH; i++)
		    test_spis_rx_data[i] = 0x09;
